    ...
    public class UnprotectFile : PSCmdlet {
        ...
        public string Path {
            ...
            set {
                ...
                if (!File.Exists(path)) {
                    throw new FileNotFoundException($"The file path indicated for the '{nameof(Path)}' parameter ({Path}) does not exist");
                }
            }
        }
        ...
        private void DoWork() {
            ...
            if (Erase > 0) {
                if (!Quiet || Util.WarnConfirm($"You have set the {nameof(Erase)} argument to $True. This will permanently delete the file '{Path}' from disk.", Constants.Affirm)) {
                    wipeTimesToWrite = Erase;
                }
            }

            Alias.DecryptFile(Path, Output, wipeTimesToWrite);
            StringBuilder Expression = new StringBuilder($"The file '{Path}' was successfully decrypted. The recovered file name is '{Output}'");
