...
namespace X509CryptoPOSH {
    ...
    public class InstallX509CryptoCertificate : PSCmdlet {
        ...
        private void DoWork() {
            ...
            if (null != Alias) {
                if (!Overwrite || !Util.WarnConfirm($"An existing {nameof(X509Alias)} with the name '{Name}' exists in the {Context.Name} {nameof(X509Context)}. OK to overwrite?", Constants.Affirm)) {
                    throw new X509CryptoException($"Could not import the certificate. An {nameof(X509Alias)} with the name '{Name}' exists in the {Context.Name} {nameof(X509Context)}");
                }
            }
